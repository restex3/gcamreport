#' Allocate National Crop Production to Provinces (Simplified Version)
#'
...
#' @export
allocate_crop_production_simple <- function(
    prj,
    scenario,
  ...
  if (allocation_method == "uniform") {
    ...
    # statistical_shares should have columns: crop, province_code, share
    provincial_prod <- national_prod %>%
      dplyr::left_join(
        statistical_shares,
        by = c("sector" = "crop")
...
#' Create Template for Statistical Allocation Shares
#'
#' Creates a template CSV file for entering provincial allocation shares
#' from statistical yearbook data.
#'
#' @param output_file Path to output CSV file
#' @export
create_statistical_shares_template <- function(output_file = "provincial_crop_shares_template.csv") {

  province_mapping <- load_gcam2gains_mapping("provinces")
  crop_mapping <- load_gcam2gains_mapping("crops")

  # Create template with all crop-province combinations
  template <- tidyr::crossing(
    crop = crop_mapping$gcam_crop,
    ...
    dplyr::mutate(
      share = NA_real_,
      notes = "Enter provincial share (0-1). Shares for each crop should sum to 1.0"
    ) %>%
    dplyr::select(crop, province_code, province_name_en, share, notes)
  ...
  cat("For each crop, provincial shares should sum to 1.0\n")
