...
When running `launch_gcamreport_ui(data_path = "path/to/your/data.RData", GCAM_version = 'v7.1')`, you might get this message:
    ...
    removeClass, show
    Error:
    Could not find the bundled gcamreport UI app.
    Reinstall the package or run devtools::load_all() from the repository root.
...
**Possible solution 2**: if you are developing locally, make sure that you launched the package from the repository root with `devtools::load_all(".", reset = TRUE)`. In addition:
...
**Possible solution 2**: if you are working from a cloned repository, reload the package from the repository root with `devtools::load_all(".", reset = TRUE)` before calling `launch_gcamreport_ui()`.
